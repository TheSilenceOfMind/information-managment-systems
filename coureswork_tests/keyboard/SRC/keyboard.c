...
/**----------------------------------------------------------------------------
                        TIMER_KB()
-------------------------------------------------------------------------------
Обработчик прерывания для клавиатуры (установлен на external Interrrupt 1)

Вход:       нет
Результат:  занесение символа в буфер (если таковой был считан), инициализация
            таймера
----------------------------------------------------------------------------- */
void TIMER_KB(void) __interrupt( 2 )
{
    ScanKBOnce();
    TH0 = 0xFE; // Инициализация таймера
    TL0 = 0xB0; // частота опроса = 10кГц (раз в 10 мс)
    ...
    // It's possible that:
    // для предотвращения занесения в буфер большого числа символов при зажатых
    //  клавишах можно использовать задержку в обработчике прерывания,
    //  например, delay(1000)
}
